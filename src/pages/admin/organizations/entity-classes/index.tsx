...
import { deleteEntityClasse, fetchEntityClasses } from 'src/api/entities/entityclasse'
import { TentityClass } from 'src/configs/traslationFields'
import { PERMISSIONS, col_entity_class } from 'src/constants'
...
function Entity_Classes() {
  ...
  const { data, isLoading, refetch, isFetching } = useQuery({
    queryKey: ['entityclasses', pagination, name],
    queryFn: async () => {
      const res = await fetchEntityClasses({
        ...pagination,
        name: name
      })
  ...
  const handleDeleteRow = async id => {
    ...
    const res = await deleteEntityClasse(id)
    ...
    <Grid container spacing={3}>
          ...
          <SectionTitle
            ...
            addButton={
              <AddButton
                ...
                text={t('Add Entity Class')}
              />
            }
          />
          <CardContent>
            <DynamicTable
              columns={col_entity_class}
              data={data?.data || []}
      ...
      {modalData && (
        <TranslationComponentModal
          ...
          path='pbf-organization-entityclasses'
        />
      )}
    </Grid>
  )
...
Entity_Classes.acl = [PERMISSIONS.entityClass.read]
export default Entity_Classes
