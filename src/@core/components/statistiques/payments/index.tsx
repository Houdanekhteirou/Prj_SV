...
const QualiteStatistiques = () => {
  ...
  const handleRemoveVisualization = index => {
    setFilterData({
      ...filterData,
      visualizations: filterData.visualizations.filter((_, i) => i !== index)
    })
  }
    ...
    <div>
      <div className='flex flex-col lg:flex-row justify-start p-6 lg:p-10 gap-6 lg:gap-10'>
        <div className='flex flex-col gap-6 lg:gap-10 w-full lg:w-1/3 justify-between'>
          ...
          {/* <div className='flex gap-6 lg:gap-10 flex-col'>
            <div className='flex gap-2 items-center'>
              <img src='/icons/HealthPBF-C-icons_s-stats-bar.svg' alt='map-marker' width={60} />
              <Typography
                style={{
                  fontSize: '1.6rem',
                  fontWeight: 'bold',
            ...
            <div className='flex gap-2 items-center flex-wrap'>
              {filterData.visualizations.map((visualization: any, index: number) => (
                <p
                  key={index}
                  className='rounded-full p-2 sm:p-4 flex items-center gap-4'
                  style={{
        ...
        <div className='flex flex-col gap-6 lg:gap-10 w-full lg:w-2/3 px-6 lg:px-10'>
          ...
          {/* <div>
            <Typography sx={{ fontWeight: 'bold', fontSize: '1.2rem' }} fontWeight='bold'>
              {t('MODÈLE DE VISUALISATIONS')}
            </Typography>
            <div className='flex flex-col md:flex-row gap-1'>
              <FormControl className='w-3/5 md:w-full'>
                <InputLabel id='indicateur-select'>{t('Choisir un modèle de visualisation')}</InputLabel>
                <Select
                  id='select-indicateur'
                  labelId='indicateur-select'
                  size='medium'
                  ...
                  onChange={e => setVisualizationModel(e.target.value)}
                  IconComponent={() => <IconifyIcon icon='fa-solid:chevron-down' color='#01A753' />}
                >
                  <MenuItem value={''}>{t('None')}</MenuItem>
                </Select>
