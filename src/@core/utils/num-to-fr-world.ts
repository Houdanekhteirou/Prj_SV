...
export const convertNumberToFrenchWords = input => {
  // cleaning spaces
  ...
  let res = ''
  if (input.includes(',')) {
    // if number has a comma, separate it and handle each part alone
    let tmp = input.split(',')
    ...
    for (let i = 0; i < postCommaInput; i++) {
      if (postCommaInput[i] == '0') zeros += 'zero '
      else break
    }
...
const cleanInput = arg => {
  arg = arg.trim()
  arg = arg.replace(/\s/g, '')
  arg = arg.replace(/\./g, ',')
  if (arg.length > NUMBER_MAX_LENGTH) return -1
  else return arg
}

const process = (
  input // main function that will start the conversion
) => {
  let prefix = ''
...
const processTensAndUnits = arg => {
  ...
  if (tens == '0') {
    ...
    if (units === '7' || units === '8' || units === '9' || units === '0') {
      ret = TEN
      if (units != '0') ret += '-' + numbersMap.get(units)
    } else {
      switch (units) {
        case '1':
          ret = ELEVEN
    ...
    if (tens === '2' || tens === '3' || tens === '4' || tens === '5' || tens === '6') {
      ...
      if (units === '1') ret += ' et ' + numbersMap.get('1')
      else if (units != '0') ret += ' ' + numbersMap.get(units)
    } else {
      if (tens == '7') {
        ret += 'soixante'
      ...
      if (tens === '8') ret += ' ' + numbersMap.get(units)
      else ret += ' ' + processTensAndUnits('1' + units)
    }
  }
  return ret
}
...
const processHundreds = arg => {
  ...
  if (arg == '0') return ret
  if (arg === '1') ret += ' cent'
  else ret += numbersMap.get(arg) + ' cents'
  return ret
}

const processThousands = arg => {
  let ret = ''
  if (arg === '000') return ret
  if (arg === '001') ret += ' mille'
  else ret += processHundreds(arg.charAt(0)) + ' ' + processTensAndUnits(arg.substring(1, 3)) + ' milles'
  return ret
}
...
const processMillions = arg => {
  let ret = ''
  if (arg === '000') return ret
  if (arg === '001') ret += ' un million'
  else ret += processHundreds(arg.charAt(0)) + ' ' + processTensAndUnits(arg.substring(1, 3)) + ' millions'
  return ret
}
...
const processMilliards = arg => {
  let ret = ''
  if (arg === '000') return ret
  if (arg === '001') ret += ' un milliard'
  else ret += processHundreds(arg.charAt(0)) + ' ' + processTensAndUnits(arg.substring(1, 3)) + ' milliards'
  return ret
}
...
const processBillions = arg => {
  let ret = ''
  if (arg === '000') return ret
  if (arg === '001') ret += ' un billion'
  else ret += processHundreds(arg.charAt(0)) + ' ' + processTensAndUnits(arg.substring(1, 3)) + ' billions'
  return ret
}
...
const processTrillions = arg => {
  let ret = ''
  if (arg === '000') return ret
  if (arg === '001') ret += ' un trillion'
  else ret += processHundreds(arg.charAt(0)) + ' ' + processTensAndUnits(arg.substring(1, 3)) + ' trillions'
  return ret
}
