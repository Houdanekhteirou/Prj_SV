...
export class NumberSection {
  ...
  private getWordByNumberSectionIndex(part: string, numberSectionIndex: number): string | null {
    ...
    if (numberSectionIndex == 0) {
      ...
      if (partAsNumber >= 3 && partAsNumber <= 10) {
        word = partWord += this.arabicWordConfig.numbers[`3e${numberSectionIndex * 3}-1e${numberSectionIndex * 3 + 1}`]
  ...
  private getWordFromTwentyToNinetyNine(tensGroup: string) {
    ...
    if (tensNum == 2) {
      ...
      tensWord = this.arabicWordConfig.numbers[tensChar] + this.arabicWordConfig.getAll().tensPrefix
    }
    if (singularNum == 0) {
      return tensWord
    ...
    return (
      singularWord + (singularWord ? ' ' + this.arabicWordConfig.getAll().numberSectionsDelimiter + ' ' : '') + tensWord
    )
  }
}
...
export class ArabicWord {
  ...
  process(num: number): string | ProcessResult {
    ...
    if (this.config.getAll().strict) {
      ...
      if (!!sections[0]) {
        result.base = this.numberSection.process(sections[0]).join(` ${this.delimiter} `)
      }
      if (!!sections[1]) {
        result.delimiter = this.config.getAll().delimiter
        result.reminder = this.numberSection.process(sections[1]).join(` ${this.delimiter} `)
      }
      return result
    } else {
...
export function converNumberToArabicWorlds(number: number): string | ProcessResult {
